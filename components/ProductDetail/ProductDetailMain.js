...
export default function ProductDetailMain({ product }) {
    ...
    const totalProductPrice = useMemo(() => {
        ...
        if (gaugeList?.length === 0) {
            // if (guageList < 1) {
            return (
                parseFloat(product?.pricing?.at(0)?.price ?? 0) *
                (quantity) ?? 0
            );
        } else if (activeFinish?.id && parseFloat(length) > 0) {

            return (
                parseInt(activeFinish?.price) *
                parseFloat(activeLength) *
                (quantity)
            );
        } else {
            return 0;
        }
    }, [activeFinish?.id, activeFinish?.price, activeLength, cart, gaugeList?.length, length, product?.pricing, quantity]);
        ...
        >
            ...
            >
                <div className="w-full">
                    ...
                    {product?.pricing?.at(0)?.gauge_size && <div className=' flex gap-4 items-center'>
                        {(
